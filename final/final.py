...
def detrmineCompareativeValue(compare, compareTo):
    ...
    return abs(sum(compareValues) - sum(compareToValues))

def shortenString(string, maxSeperator):
    string = str(string)
    string = re.sub(f"[(@*&?].*[)@*&?]", "", string)
    if ',' in string:
        splitString = string.split(",")
        splitString = splitString[:maxSeperator]
        i = 0
        for s in splitString:
            splitString[i] = colonMinimizer(dashStringMinimizer(s))
            i += 1
        ...
        string =  colonMinimizer(dashStringMinimizer(string))
        
    return string

def dashStringMinimizer(string):
    if " - " in string:
        splitString = string.split(" - ")
        string = min(splitString, key=len)
    return string

def colonMinimizer(string):
    if ":" in string:
...
def printMultiArray(rowNames, rowArrays, multiArray, printTo):
    ...
    for i in range(0, len(multiArray)):
        finalPrintedCollumn = str(i+1) + " "
        for j in range(0, len(multiArray[i])):
            multiArray[i][j] = shortenString(multiArray[i][j], 3)
...
def menu(individualArrays, multiArray):
    ...
    if enter == "y":
        ...
        while searchAgain == 'y':
            ...
            searchAgain = input("\nDo you want to search the library again? (y,n)").lower()
...
menu([titles, prices, ratings, authors, yearsPublished, genres, urls], fileValues)