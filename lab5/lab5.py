...
def binarySearch(searchTerm, searchList):
    ...
    while (min < max and str(searchTerm.lower()) != str(searchList[mid]).lower()):
        bin_count += 1
        if str(searchTerm).lower() < str(searchList[mid]).lower():
            max = mid - 1
        else:
            min = mid + 1
...
#function that asks for search term based on context and preforms binary search, then prints result
def compleatBinarySearch(context, searchList, lists, spacing):
    ...
    for i in range(0, len(lists)):
        printListString += f"{str(lists[i][mid]):8} "
    print("\n"+printListString+"\n")
...
while searchFile == 'y':
    ...
    #Defines choice and runs diffrent programs based on choice
    choice = int(menu())
    if choice == 1:
        #prints entire file
        ...
        #prints list of people with class
        classToSearch = input("\nWhat class would you want to search for?: ")
        foundStudents = seqSearch(classToSearch, [class1, class2, class3])

        print(f"\n {'ID':5} \t {'Last Name':8} \t {'First Name':10} \t {'Class 1':8} \t {'Class 2':8} \t {'Clsss 3':8}")

        for val in foundStudents:
            print(f"\n {id_stud[val]:5} \t {lname[val]:8} \t {fname[val]:10} \t {class1[val]:8} \t {class2[val]:8} \t {class3[val]:8}")
    else:
        searchFile = 'n'
        print("GoodBye!")
